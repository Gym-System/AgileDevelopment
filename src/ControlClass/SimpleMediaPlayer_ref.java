 ...
 */
public class SimpleMediaPlayer_ref extends AnchorPane {

//TODO 修改player.fxml 使其为自适应的大小，使用AnchorBar或者修改底部工具栏高度


    private static SimpleMediaPlayer_ref simpleMediaPlayer;   //创建实例保存到私有域中
    private PlayerController_ref controller;     //储存每个实例的控制器对象


    protected PlayerController_ref getController(){   //提供控制器对象的调用接口
    ...
    private SimpleMediaPlayer_ref(String mediaUrl){
        try {
            FXMLLoader fxmlloader = new FXMLLoader(getClass().getResource("../BoundaryClass/user_recording_video.fxml"));
    ...
    //弹窗式调用：默认大小800*600
    public static SimpleMediaPlayer_ref popup(String mediaUrl){
